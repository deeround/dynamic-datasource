    ...
    private String getDefaultDataSourceAttr(Object targetObject) {
        ...
        if (!Proxy.isProxyClass(targetClass)) {
            ...
            while (currentClass != Object.class) {
                String datasourceAttr = this.findDataSourceAttribute(currentClass);
        ...
        if (mpEnabled) {
            final Class<?> clazz = this.getMapperInterfaceClass(targetObject);
            if (clazz != null) {
                String datasourceAttr = this.findDataSourceAttribute(clazz);
                if (datasourceAttr != null) {
                    return datasourceAttr;
                }
                // 尝试从其父接口获取
